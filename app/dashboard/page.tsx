...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const loadPolicies = async () => {
      ...
      const records = await policyRepository.listByWallet(
  ...
  const totalCoverage = useMemo(
    () => displayedPolicies.reduce((sum, policy) => sum + policy.coverage, 0),
    [displayedPolicies],
  );
  const totalPayouts = useMemo(
    () =>
      displayedPolicies
        .filter((policy) => computePolicyStatus(policy) === "paid")
        .reduce((sum, policy) => sum + policy.coverage, 0),
    [displayedPolicies],
  );
  const totalPremiums = useMemo(
    () => displayedPolicies.reduce((sum, policy) => sum + policy.premium, 0),
    [displayedPolicies],
  );
  const paidPolicies = useMemo(
    () =>
      displayedPolicies.filter(
        (policy) => computePolicyStatus(policy) === "paid",
      ),
    [displayedPolicies],
  );
    ...
    () => [
      {
        ...
        value: `${totalCoverage} ALGO`,
        sub: `Across ${displayedPolicies.length} policies`,
        icon: Shield,
        color: "cyan" as const,
      },
      {
        label: "Total Payouts",
        value: `${totalPayouts} ALGO`,
        sub: `${paidPolicies.length} claim${paidPolicies.length === 1 ? "" : "s"} triggered`,
        icon: Zap,
        color: "emerald" as const,
      },
      {
        label: "Total Premiums",
        value: `${totalPremiums.toFixed(2)} ALGO`,
        sub: "Paid from wallet on buy",
        icon: TrendingUp,
      ...
      },
      {
        ...
        value: avgSettlement,
        sub: "Measured from buy to payout tx",
        icon: Clock,
  ...
  const activityItems = useMemo(
    () => buildActivityItems(displayedPolicies),
    [displayedPolicies],
  );
      ...
      <div className="min-h-screen pt-24 flex items-center justify-center px-4">
        <motion.div {...fadeUp()} className="text-center max-w-md">
          ...
          <p className="text-zinc-400 text-sm mb-6">
            Your policies, payouts, and activity — all on-chain.
          </p>
          <Button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid md:grid-cols-3 gap-6">
            ...
            >
              <div className="flex items-center justify-between p-5 border-b border-white/5">
                ...
                <div className="flex items-center gap-2">
                  {isUsingDemoData && (
                    <Badge className="bg-violet-400/10 text-violet-400 border-violet-400/20 text-xs">
                      Demo data
                    </Badge>
                  )}
                  <Badge className="bg-zinc-800 text-zinc-300 border-zinc-700 text-xs font-medium">
                    {displayedPolicies.length} total
                  </Badge>
                </div>
              </div>
              {!displayedPolicies.length && (
                <div className="p-6 text-sm text-zinc-400">
                  {isLoadingPolicies
                    ? "Loading policies..."
                    : "No policies found yet. Buy one from the app page to see live status here."}
                </div>
              )}
              <div className="divide-y divide-white/5">
                {displayedPolicies.map((policy, i) => {
                  ...
                  const explorerTx = isUsingDemoData
                    ? undefined
                    : getExplorerTx(policy);
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              ...
                              <span className="text-xs text-zinc-400">
                                {formatDateLabel(policy.createdAt)}
                              </span>
                              <span className="text-zinc-500 text-xs">·</span>
                              <span className="text-xs text-zinc-400">
                                {policy.id.slice(-8).toUpperCase()}
                              </span>
                            </div>
                          </div>
                        </div>
          ...
          <div className="space-y-4">
              ...
              <div className="p-4 space-y-1">
                {!activityItems.length && (
                  <p className="text-xs text-zinc-400 px-3 py-2">
                    No activity yet. Policy events will appear here after a buy.
                  </p>
                )}
            ...
            <motion.div
              {...fadeUp(0.4)}
              className="glass rounded-2xl p-5 border border-violet-400/10"
            >
              <div className="flex items-center gap-2 mb-3">
                <BarChart3 className="w-4 h-4 text-violet-400" />
                <p className="text-sm font-medium text-zinc-300">Analytics</p>
                <Badge className="bg-violet-400/10 text-violet-400 border-violet-400/20 text-[10px] ml-auto">
                  Soon
                </Badge>
              </div>
              <p className="text-xs text-zinc-400 leading-relaxed">
                Historical payout rates, route delay heatmaps, and portfolio
