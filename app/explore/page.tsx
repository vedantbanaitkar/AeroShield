    ...
    <div className="min-h-screen pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <motion.div {...fadeUp()} className="text-center mb-16">
          <Badge className="bg-violet-400/10 text-violet-400 border-violet-400/20 mb-4">
            Product Suite
          </Badge>
          <h1
          ...
          <p className="text-slate-700 max-w-2xl mx-auto text-lg leading-relaxed">
            AeroShield is not just flight insurance. It's a parametric insurance
            infrastructure layer — any real-world data point can become a
            trigger. Here's what we're building.
          </p>
        </motion.div>
        ...
        <div className="space-y-6">
          {products.map((product, i) => (
            ...
            >
              <div
                className={`absolute inset-0 bg-gradient-to-r ${product.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl`}
              />
                ...
                <div className="grid md:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2">
                    <div className="flex items-start gap-4 mb-5">
                      <div
                        className={`w-12 h-12 rounded-xl bg-gradient-to-br ${product.gradient} border ${product.border} flex items-center justify-center flex-shrink-0`}
                      >
                        <product.icon
                          className={`w-6 h-6 ${colorMap[product.color]}`}
                        />
                      </div>
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <Badge
                            variant="secondary"
                            className="text-slate-700 border-zinc-500 text-xs font-medium"
                          >
                            {product.category}
                          </Badge>
                          {product.status === "live" ? (
                            ...
                            <Badge className="bg-zinc-800 text-slate-200 border-zinc-600 text-xs font-medium">
                              Coming Soon
                            </Badge>
                          )}
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {product.features.map((feature) => (
                        ...
                        >
                          <CheckCircle2
                            className={`w-3.5 h-3.5 flex-shrink-0 ${colorMap[product.color]}`}
                          />
                          {feature}
                        </div>
                      ))}
                  ...
                  <div className="glass rounded-xl p-5 space-y-3 self-start">
                    <p className="text-xs uppercase tracking-widest text-slate-700 font-medium mb-3">
                      Policy specs
                    </p>
                    {[
                      ...
                      >
                        <spec.icon
                          className={`w-3.5 h-3.5 mt-0.5 flex-shrink-0 ${colorMap[product.color]}`}
                        />
                        <div>
                    ...
                    {product.status === "live" ? (
                      <Link href="/app" className="block mt-4">
                        <Button
                          size="sm"
                          className={`w-full bg-cyan-400 hover:bg-cyan-300 text-black font-bold gap-2`}
                        >
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        className="w-full mt-4 border-white/10 text-slate-700 gap-2"
                        disabled
                      >
                        Notify me when live
        ...
        <motion.div
          {...fadeUp(0.3)}
          className="mt-16 glass rounded-2xl p-10 text-center border border-violet-400/10 relative overflow-hidden"
        >
          ...
          <div className="relative">
            ...
            <p className="text-slate-700 max-w-xl mx-auto mb-8 text-sm leading-relaxed">
              AeroShield exposes a composable smart contract API. Plug in any
              oracle data source, set your own trigger logic, and launch a
              parametric insurance product in days — not months.
            </p>
            <div className="flex flex-wrap gap-3 justify-center">
              ...
              <Button
                variant="outline"
                className="border-white/10 text-slate-700 hover:text-slate-900 gap-2"
              >
