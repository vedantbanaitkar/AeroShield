...
async function main() {
  try {
    ...
    console.log('✓ Created policies for Wallet 3 (Inactive session, Paid policy)');
    ...
    console.log(`   • Active Sessions: ${allSessions.filter(s => s.isActive).length}`);
    console.log(`   • Policies: ${allPolicies.length}`);
    console.log(`   • Active Policies: ${allPolicies.filter(p => p.status === 'active').length}`);
    console.log(`   • Paid Policies: ${allPolicies.filter(p => p.status === 'paid').length}`);
